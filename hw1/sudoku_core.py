...
def propagate(sudoku_possible_values,k):
    ...
    edges = get_edges(vertices, k) # 810 for k = 3
    ...
    while len(edges) > 1:
        ...
        for (v1, v2) in edges:
            ...
            if len(p1) == 1 and p1[0] in p2:
                ...
                if len(sudoku_possible_values[row2][col2]) == 1 and not (v1, 
                    v2) in new_edges:
            ...
            if len(p2) == 1 and p2[0] in p1:
                ...
                if len(sudoku_possible_values[row1][col1]) == 1 and not (v1, 
                    v2) in new_edges:
                    new_edges.append((v1, v2))
...
def solve_sudoku_ASP(sudoku,k):
    ...
    # Universily quantified variables; number for each vertex, already filled in 
    # value must also hold
    for n1 in range(1, num_nums+1):
...
def get_edges(vertices,k,remove_double=True):
    edges = []
    # Obtain edges between vertices in rows and in columns by permuting
    for row_col in range(k*k):
        col_edges = list(itertools.permutations(vertices[:, row_col], 2))
        edges.extend(col_edges)
        row_edges = list(itertools.permutations(vertices[row_col, :], 2))
